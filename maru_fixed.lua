...
local rf_craft = net:WaitForChild("RF/Craft")
...
local rf_jobsremotefunction = net:WaitForChild("RF/JobsRemoteFunction")
local rf_jobtoolabilities = net:WaitForChild("RF/JobToolAbilities")
...
Window = Library:CreateWindow({ Title = "HDZHub", Subtitle = "- by hoang", Image = "rbxassetid://96779554580445" });
wait(1);
Library:Notify({ Title = "HDZHub", Description = "The UI automatically hides once executed.\nPress the button at the bottom-left of the screen to show the GUI.", Duration = 3 });
wait(0);
...
ReportIdeas:AddButton({ Title = "Send To Server Developer", Callback = function()
		...
		local r = { username = "HDZHub BOT", avatar_url = "https://cdn.discordapp.com/attachments/...", embeds = { {
						title = Y,
						color = C,
						...
						footer = { text = "HDZHub \226\128\162 " .. p, icon_url = "https://cdn.discordapp.com/attachments/..." },
...
MeterialFarm:AddToggle("MasteryStart", { Title = "Farm Mastery", Default = false, Callback = function(p)
		...
		if not _G.StartFarn then
			Library:Notify({ Title = "HDZHub", Description = "Open Start Farm Plz!", Duration = 3 });
		end;
	end });
...
DevilFarm:AddToggle("DevilFarmHopServer", { Title = "Teleport to Fruit [Hop Server]", Default = false, Callback = function(p)
		...
		if not _G.TwFruits then
			Library:Notify({ Title = "HDZHub", Description = "Open Teleport to Fruit Plz!", Duration = 3 });
		end;
	end });
spawn(function()
	while wait(QH) do
...
RipIndraBoss:AddToggle("EliteHunterFarmHop", {
	...
	Callback = function(p)
		_G.EliteHop = p;
		if not _G.TwFruits then
			Library:Notify({ Title = "HDZHub", Description = "Open Auto Elite Hunter Plz!", Duration = 3 });
		end;
	end,
});
spawn(function()
...
SoulReaperBoss:AddToggle("SoulReaperHop", { Title = "Auto Soul Reaper [ Hop Server ]", Default = false, Callback = function(p)
		...
		if not _G.AutoHytHallow then
			Library:Notify({ Title = "HDZHub", Description = "Open Auto Soul Reaper Plz!", Duration = 3 });
		end;
	end });
spawn(function()
	while wait(QH) do
...
DoughKingBoss:AddToggle("DoughKingHop", { Title = "Auto Dough King [ Hop Server ]", Default = false, Callback = function(p)
		...
		if not _G.AutoMiror then
			Library:Notify({ Title = "HDZHub", Description = "Open Auto Dough King Plz!", Duration = 3 });
		end;
	end });
spawn(function()
	while wait(QH) do
...
DarkbeardBoss:AddToggle("DarkBreadHop", { Title = "Auto Darkbread [ Hop Server ]", Default = false, Callback = function(p)
		...
		if not _G.Auto_Def_DarkCoat then
			Library:Notify({ Title = "HDZHub", Description = "Open Auto Darkbread Plz!", Duration = 3 });
		end;
	end });
spawn(function()
	local p = .1;
...
BerryFarm:AddToggle("BerryFarmHop", { Title = "Hop Find Berry", Default = false, Callback = function(p)
		...
		if not _G.AutoBerry then
			Library:Notify({ Title = "HDZHub", Description = "Open Auto Berry Plz!", Duration = 3 });
		end;
	end });
local function bH()
...
ChestFarm:AddToggle("ChestHopAuto", { Title = "Auto Chest Hop", Default = false, Callback = function(p)
		...
		if not _G.AutoFarmChest then
			Library:Notify({ Title = "HDZHub", Description = "Open Auto Chest Plz!", Duration = 3 });
		end;
	end });
local function p5()
	local p = game:GetService("CollectionService");
...
ObservationFarm:AddToggle("ObservationHop", { Title = "Farm Observation [ Hop Server ]", Default = false, Callback = function(p)
		...
		if not _G.obsFarm then
			Library:Notify({ Title = "HDZHub", Description = "Open Auto Observation Plz!", Duration = 3 });
		end;
	end });
spawn(function()
	while wait(.2) do
...
GetItems:AddToggle("AutoHopColorAndSword", { Title = "Hop Server [Haki Color or Legendary Sword]", Default = false, Callback = function(p)
		...
		if not _G.Auto_Buy_Legendary_Sword and not _G.HakiColorBusoBuy then
			Library:Notify({ Title = "HDZHub", Description = "Open Buy Haki Color or Legendary Sword Plz!", Duration = 3 });
		end;
	end });
local function K5(p)
